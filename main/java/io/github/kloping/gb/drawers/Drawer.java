...
//
    ...
    public static String createImage(String... sss) {
        ...
        for (int i = 0; i < sss.length; i++) {
            g.setColor(COLORS[Utils.RANDOM.nextInt(COLORS.length)]);
            g.drawString("◎" + Utils.filterBigNum(sss[i]), 10, (i + 2) * 40);
        }
        g.setColor(BORDER_COLOR);
    ...
    public static String drawStringOnTwoColumns(String... ss) {
        ...
        for (int i = 0; i < ss.length; i++) {
            if (i % 2 == 0) {
                g.setColor(COLORS[Utils.RANDOM.nextInt(COLORS.length)]);
                g.drawString(Utils.filterBigNum(ss[i]), 15, 60 + (i) * 20);
                g.setColor(COLORS[Utils.RANDOM.nextInt(COLORS.length)]);
                if (ss.length > i + 1) g.drawString(Utils.filterBigNum(ss[i + 1]), width / 2, 60 + (i) * 20);
            }
        }
        g.dispose();
        try {
    ...
    public static String drawLines(String... sss) {
        return createImage(sss);
    }

    public static String drawLines(boolean k, String... strings) {
        return  createImage(false, strings);
    }

    public static String drawLine(String string) {
        return createFont(string);
    }

    public static String getImageFromStringsOnTwoColumns(String... strings) {
        return drawStringOnTwoColumns(strings);
    }
    ...
    public static String drawHh(Integer mid, List<Integer> ids) throws Exception {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            for (int i1 = 0; i1 < ids.size(); i1++) {
                BufferedImage hi = (BufferedImage) ImageManager.getImageById(ids.get(i1));
