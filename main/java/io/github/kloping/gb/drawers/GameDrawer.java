...
import io.github.kloping.gb.drawers.entity.GameMap;
import io.github.kloping.gb.drawers.entity.MapPosition;
        ...
        for (MapPosition position : map.getPositions()) {
            ...
            switch (position.getType()) {
                case p:
                    ...
                    g.drawImage(ImageDrawerUtils.getImageByUrl2Size(new URL(position.getArg().toString()), 50, 50)
                            , x1, y1, null);
                    break;
                default:
                    break;
        ...
        for (MapPosition position : map.getPositions()) {
            ...
            switch (position.getType()) {
                case p:
                    x1 = (position.getX() - 1) * 50;
                    y1 = (position.getY() - 1) * 50;
                    g.drawImage(ImageDrawerUtils.getImageByUrl2Size(
                            new File(position.getArg().toString()).toURL(), 50, 50), x1, y1, null);
                    break;
                default:
                    break;
            }
        ...
        for (MapPosition position : map.getPositions()) {
            ...
            switch (position.getType()) {
                case p:
                    x1 = (position.getX() - 1) * 50;
                    y1 = (position.getY() - 1) * 50;
                    BufferedImage item0 = (BufferedImage) ImageDrawerUtils.getImageByUrl2Size(new File(position.getArg().toString()).toURL(), 50, 50);
    ...
    public static String drawerDynamic(GameMap map, int od, String end, File outFile) throws Exception {
        ...
        for (int index = 0; index < RATE_LIST.size(); index++) {
            if (index > 5 && n0 < 0) {
                ...
                BufferedImage o0 = (BufferedImage) ImageDrawerUtils.getImageByColor2Size(
        ...
        x1 = 1 * 50;
        y1 = 1 * 50;
        o0 = (BufferedImage) ImageDrawerUtils.getImageByUrl2Size(new File(end).toURL(), 50, 50);
