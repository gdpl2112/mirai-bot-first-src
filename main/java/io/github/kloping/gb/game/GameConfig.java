...
public class GameConfig {
    public final Map<Integer, String> ID_2_NAME_MAPS = new ConcurrentHashMap<>();
    public final Map<Integer, String> ID_2_INTRO_MAPS = new ConcurrentHashMap<>();
    public final Map<Integer, String> NAME_2_INTRO_MAPS = new ConcurrentHashMap<>();
    public final Map<String, Integer> NAME_2_ID_MAPS = new ConcurrentHashMap<>();
    public final Map<Integer, Long> ID_2_SHOP_MAPS = new ConcurrentHashMap<>();
    public final Map<Integer, Integer> ID_2_WEA_MAPS = new ConcurrentHashMap<>();
    public final Map<Integer, Integer> ID_2_WEA_O_NUM_MAPS = new ConcurrentHashMap<>();
    public final Map<Integer, Integer> WH_2_TYPE = new ConcurrentHashMap<>();
    ...
    private void initIntro() {
        ...
        ID_2_INTRO_MAPS.put(116, "冷却药水,使指定一个魂技减少" + FinalConfig.OBJ116_VALUE / 60000 + "分钟冷却,使用示例,使用冷却药水2");
        ID_2_INTRO_MAPS.put(117, "大冷却药水,使所有魂技立刻完成冷却,但每日只能使用2次");
     ...
     * @return
     */
    public static boolean isJTop(WhInfo w) {
        long lev = w.getLevel();
        lev++;
        boolean k1 = lev % 10 == 0;
        boolean k2 = w.getXp() >= w.getXpL();
        return k1 && k2;
    }

    public static boolean isJTop0(WhInfo w) {
        long lev = w.getLevel();
        lev++;
        boolean k1 = lev % 10 == 0;
        return k1;
    ...
    public static long getLtoGhsL(long l) {
        if (l < 10) return Utils.randA(100, 200);
        else if (l < 20) return Utils.randA(200, 1000);
        else if (l < 30) return Utils.randA(1000, 10000);
        else if (l < 40) return Utils.randA(10000, 20000);
        else if (l < 60) return Utils.randA(20000, 40000);
        else if (l < 80) return Utils.randA(40000, 80000);
        else if (l < 90) return Utils.randA(80000, 120000);
        else if (l < 95) return Utils.randA(120000, 200000);
        else if (l < 100) return Utils.randA(200000, 1000000);
        else if (l < 110) return Utils.randA(300000, 1500000);
        else if (l < 120) return Utils.randA(800000, 2000000);
        else if (l < 130) return Utils.randA(900000, 3000000);
        else if (l <= 151) return Utils.randA(990000, 10010000);
        else return 1L;
    }
    ...
    public static long getAArtt(int level) {
        if (level < 10) return Utils.randA(80, 120);
        else if (level < 40) return Utils.randA(290, 315);
        else if (level < 60) return Utils.randA(900, 1100);
        else if (level < 80) return Utils.randA(2500, 2600);
        else if (level < 90) return Utils.randA(3900, 4000);
        else if (level < 95) return Utils.randA(39000, 40000);
        else if (level < 100) return Utils.randA(95000, 100000);
        else if (level < 110) return Utils.randA(179500, 190000);
        else if (level < 120) return Utils.randA(280000, 285000);
        else return Utils.randA(160000, 166667);
    }
    ...
    public static Long randFloatByte1(Long v) {
        return Long.valueOf(Utils.randLong(v, 0.87f, 1.18f));
    }
